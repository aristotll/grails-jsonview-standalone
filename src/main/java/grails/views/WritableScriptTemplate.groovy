...
class WritableScriptTemplate implements Template {
     ...
     */
    final Class<? extends WritableScript> templateClass
    /**
    ...
    protected void initModelTypes(Class<? extends WritableScript> templateClass) {
        ...
        if (field != null) {
            ...
            def info = Introspector.getBeanInfo(templateClass, AbstractWritableScript.isAssignableFrom(templateClass) ? AbstractWritableScript : WritableScript)

            def descriptors = info.getPropertyDescriptors()
            descriptors.each { pd ->
                def propertyName = pd.name
                def writeMethod = pd.getWriteMethod()
                if (writeMethod != null) {
                    writeMethod.setAccessible(true)
                    modelSetters.put(propertyName, new MethodSetter(pd.propertyType, writeMethod))
                } else {
                    def propertyField = ReflectionUtils.findField(templateClass, propertyName)
                    if (propertyField != null) {
                        propertyField.setAccessible(true)
